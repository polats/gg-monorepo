...
export const VhsModel = React.forwardRef<any, VhsModelProps>(
  ({ texturesSrc, draggableRigidBodyProps, groupProps, ...props }, ref) => {
    ...
    const meshRefs = useMemo(
      () => texturesSrc.map(() => React.createRef<THREE.Group>()),
      [texturesSrc]
    );
    const geometry = useMemo(() => new THREE.BoxGeometry(1.05, 1.01, 0.5), []);
            ...
            <DraggableRigidBody
              ...
              visibleMesh={
                <group ref={meshRefs[i]} {...props} dispose={null}>
                  ...
                  <mesh position={[0, -0.14, 0.3]} scale={1.8} rotation={[0, 0, Math.PI / 2]}>
                    <planeGeometry args={[0.64, 1.1]} />

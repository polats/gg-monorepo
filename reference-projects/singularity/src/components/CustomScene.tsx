...
export const ContainerScene = () => {
  ...
  const bloom = useControls({
    bloom: folder(
      {
        enabled: true,
        radius: { value: 0.92, min: 0, max: 2, step: 0.01 },
        intensity: { value: 0.79, min: 0, max: 10, step: 0.01 },
        luminanceThreshold: { value: 0.56, min: 0, max: 10, step: 0.01 },
        luminanceSmoothing: { value: 0.55, min: 0, max: 10, step: 0.01 },
      },
      { collapsed: true }
    ),
  });
  ...
  const hueSaturation = useControls({
    hueSaturation: folder(
      {
        enabled: true,
        saturation: { value: 0.26, step: 0.01 },
      },
      { collapsed: true }
    ),
  });
      ...
      {showBranding && (
        <>
          ...
          <div className="flex justify-center z-50 absolute bottom-[5vh] w-[100%]">
            <button
              className={`${GeistSans.className} bg-white/60 px-3 py-1 text-sm font-medium text-zinc-800 shadow-lg ring-1 ring-zinc-900/5  rounded-full`}
              onClick={() => {
      ...
      >
        ...
        <Suspense fallback={null}>
          ...
          >
            ...
            <ContainerModel
              position={[0, 0, -0.2]}
              enablePointer={attractorEnabled && physics.enableMouseRepulsion}
        ...
        <EffectComposer>
          <ToneMapping />
          {isAOEnabled ? (
            <N8AO quality={'medium'} distanceFalloff={1} aoRadius={1} intensity={4} />
          ) : (
            <></>
          )}
          ...
          {depthOfField.enabled ? <DepthOfField {...depthOfField} resolutionScale={0.6} /> : <></>}

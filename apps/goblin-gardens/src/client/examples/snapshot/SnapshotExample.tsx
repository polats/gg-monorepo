...
export const SnapshotExample: Demo = () => {
  ...
  useControls({
    ...
    restoreSnapshot: button(
      () => !!worldSnapshot.current && setWorld(rapier.World.restoreSnapshot(worldSnapshot.current))
    ),
  });
  ...
  return (
    <>
      <group>
        ...
        <RigidBody type="fixed" colliders="cuboid">
          <Box args={[40, 0.5, 14]} position={[18, 2, -5]} rotation={[Math.PI / 8, 0, 0]}>
            <meshStandardMaterial map={ramp} />
          </Box>
        </RigidBody>

        {balls.map((i) => (
          <RigidBody key={i} colliders="ball" position={[i * 3, 10, -10]} angularDamping={i / 10}>
            <Sphere>

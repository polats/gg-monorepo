...
export const isIpad = () => {
    ...
    if (navigator.userAgent) {
        return !!(navigator.userAgent.match(/(iPad)/) || (navigator.platform === "MacIntel" && typeof (navigator as any).standalone !== "undefined"));
    }
...
export const getDevicePixelRatio = () => {
    ...
    const mobilePixelRatio = 1.5;
    const ipadPixelRatio = .8;
    const maxPixelRatio = 1;

    if (isIpad()) {
        return ipadPixelRatio;
    }

    if (typeof window !== "undefined") {
        if (isMobile()) {
            return Math.min(mobilePixelRatio, window.devicePixelRatio);
        }

        return Math.min(maxPixelRatio, window.devicePixelRatio);
    }
